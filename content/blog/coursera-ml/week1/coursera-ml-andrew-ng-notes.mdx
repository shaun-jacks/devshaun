...
# generate 100 x values between 0 and 3
...
The code we just wrote would be the same as $Y = 7X_{1} + 2X_{0}$ with some random offset data. Where $X_{0} = 1$

We will now add the X0 term to our matrix X

```python
# concatenate 100 1s to our 100 X1 values between 0 and 1
...
Where we take the dot product of $\theta$ and $X$

Lets now define our variables for the equations:
...
So we can now infer every variable in our equations from the variables `X_matrix`, `alpha`, `theta`, `m`, and `y`.

Lets put our equation $\nabla_{\theta}J(\theta) = \frac{2}{m}X^{T}(X\theta - y)$ into a function.
...
We will now loop through each iteration, updating our theta parameters accordingly to implement this equation on every iteration:
...
Now lets plot our Linear Regression line with the new theta values
...
And plot our data with the optimized line
